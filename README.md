...
- **terragrunt**: インフラ管理 (Terraform のラッパー)。`brew install terragrunt` 等
...
#### 方法 A: Terragrunt で一括作成 (推奨)

`infra/live/local/` でデータバケット + ローカル API 用 SA + 署名 URL 用 IAM をまとめて作成できます。詳細は [infra/README.md](./infra/README.md) を参照。
...
#### Google OAuth Client ID の作成 (ローカル開発用)

ローカルではフロントが Google Identity Services で ID Token を取得し、API がそれを検証します。GCP Console で OAuth Client を作成してください。
...
3. Authorized JavaScript origins:
   - `http://localhost:5173`
4. Authorized redirect URIs: **不要** (GIS の Implicit Flow を使用)
5. 作成後に表示される **Client ID** をコピー (Client Secret はローカルでは不要)
...
> `APP_ENV=development` を設定すると、Cloudflare Access の JWT 検証の代わりに Google ID Token (Authorization: Bearer …) を検証します。
...
> API と同じ Client ID をフロント側にも入れる必要があります（ID Token の `aud` 検証のため）。
...
Google サインアップを試すだけなら **不要** (空の DB から Google ログイン→オンボーディングで新規テナント作成、で完結する)。

既存テナントへの参加フローを試したい場合などにシードを投入:
...
## サインアップ / ユーザー登録 / CSV アップロード / 日付ビュー

### ローカルのサインアップフロー

1. `pnpm dev:api` + `pnpm dev:front` を起動して http://localhost:5173 を開く
2. 「Sign in with Google」ボタンをクリック → Google アカウントを選択
3. 初回ログイン時は `/onboarding` 画面が自動で開く
   - 「新規テナントを作成」を選ぶと、自分が **admin** として作成される
   - 「既存テナントに参加」を選ぶと、既存テナント (例: seed で作った `My Tenant`) に **viewer** として加入する
4. オンボーディング完了後はホームに戻り、Header からアプリ機能 (アップロード / 日付ビュー / ユーザー登録) を利用できる

「ユーザー登録」リンクは role が `admin` のときだけ表示されます。

### 画面

| ルート          | 目的                                                                            |
| --------------- | ------------------------------------------------------------------------------- |
| `/register`     | 同テナントに新規ユーザーを追加（admin 限定）                                    |
| `/upload`       | 対象日の `data.csv` をアップロード / 上書き                                     |
| `/csv/by-date`  | 日モード（単体表示）/ 月モード（リスト）で過去アップロードを閲覧                |

### 保管パスの規約

GCS バケットに以下のパスで保管されます（**1 日 1 ファイル**、日付は JST）:

```
gs://${GCS_BUCKET_NAME}/tenant_id={tenant_id}/yyyy={YYYY}/mm={MM}/dd={DD}/data.csv
```

再アップロードは同じパスを上書きします（UI 側で確認プロンプトを出します）。履歴は DB に記録せず GCS のパス構造で表現します。

### GCS バケットの CORS

ブラウザから直接 PUT するため、データバケットの CORS で `PUT` メソッドを許可する必要があります。`infra/modules/local/storage.tf` は既に `PUT` を含む設定になっています。`infra/live/local` を初めて適用する場合 / 既存環境を更新する場合は `terragrunt apply` を再実行してください。

### curl での確認

ブラウザの devtools で `localStorage.getItem("google_id_token")` を実行して ID Token を取り出します。

```sh
TOKEN="<google id token>"

# /api/me (オンボーディング前なら needsOnboarding: true)
...
# オンボーディング (新規テナント作成 → 自分が admin)
curl -s -X POST http://localhost:8080/api/onboarding \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"action":"create","tenantName":"My Workspace"}' | jq
...
# アップロード initiate (date 省略時は JST の今日)
RES=$(curl -s -X POST http://localhost:8080/api/uploads/initiate \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"contentType":"text/csv"}')
SIGNED=$(echo "$RES" | jq -r .signedUrl)

# 取得した signedUrl に PUT (Authorization ヘッダー不要 — 署名 URL 自体に認証情報が含まれる)
curl -X PUT "$SIGNED" \
  -H "Content-Type: text/csv" \
  --data-binary @./mock_data.csv

# 日モード (yyyy + mm + dd)
curl "http://localhost:8080/api/uploads?yyyy=2026&mm=05&dd=24" \
  -H "Authorization: Bearer $TOKEN"

# 月モード (yyyy + mm のみ)
curl "http://localhost:8080/api/uploads?yyyy=2026&mm=05" \
  -H "Authorization: Bearer $TOKEN"
```

### 本番で Google ログインを有効化 (Cloudflare Access の IdP として)

本番は **アプリのコード変更は不要**。Cloudflare Zero Trust の Identity Provider に Google を登録するだけ。

1. GCP Console で OAuth Client を 1 つ作成 (Authorized redirect URI は Cloudflare が指定するもの: `https://<team-domain>.cloudflareaccess.com/cdn-cgi/access/callback`)
2. `infra/live/cloud/terraform.tfvars` に `google_oauth_client_id` と `google_oauth_client_secret` を設定
3. `cd infra/live/cloud && terragrunt apply`

これで CF Access のログイン画面に「Sign in with Google」が表示されるようになります。Cloudflare Access は引き続き `cf-access-jwt-assertion` JWT を発行し、API 側は既存の `verifyCFAccessJWT` でそのまま検証します。
...
`gcloud` / Cloudflare ダッシュボードによる手動作業の代わりに、`infra/` の **terragrunt 構成** で GCP と Cloudflare Access を一括管理できます。`infra/` はローカル用 (`local`) と本番相当 (`cloud`) で環境分割されています。

詳細なディレクトリ構成・手順は [infra/README.md](./infra/README.md) を参照。
...
infra/
├── modules/
│   ├── local/   # ローカル開発用: データ用 GCS バケット + ローカル API 用 SA + IAM
│   └── cloud/   # 本番相当: Artifact Registry, Cloud Run, LB, Cloudflare Access, Secret Manager 等
└── live/
...
- `terragrunt` がインストール済み (`brew install terragrunt` 等)
...
GCS をリモートバックエンドにする場合は `infra/live/terragrunt.hcl` の `remote_state` ブロックのコメントを外してから実行してください。

**3. Docker イメージのビルドとプッシュ**

Terragrunt でインフラを作成した後、イメージをプッシュします。

```sh
# Artifact Registry リポジトリのみ先に作成
terragrunt apply -target=google_artifact_registry_repository.api

# Docker 認証
gcloud auth configure-docker asia-northeast1-docker.pkg.dev

# イメージのビルドとプッシュ (プロジェクトルートで実行)
IMAGE=$(terragrunt output -raw artifact_registry_repo)
cd ../../..
docker build -t "${IMAGE}:latest" .
docker push "${IMAGE}:latest"
```
...
BUCKET=$(cd infra/live/cloud && terragrunt output -raw frontend_bucket_name)
...
cd infra/live/cloud
terragrunt apply -var="image_tag=latest"
```
...
#### ローカル環境 (local) のセットアップ

ローカル開発で使う最小リソース (データバケット + 署名 URL 用 SA + IAM) のみを作成します。
...
# 出力された SA で ADC を再設定 (V4 署名付き URL を発行できるようにする)
eval "$(terragrunt output -raw impersonation_login_command)"
```
...
Cloudflare Access の `self_hosted` タイプは **Cloudflare DNS でプロキシ中（orange-cloud）のドメイン**が必要です。`terragrunt apply` で取得した LB の IP を A レコードに設定します。

### 1. Terragrunt でインフラを作成し LB の IP を取得
